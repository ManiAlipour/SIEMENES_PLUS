 ...
 */
export async function extractAuthErrorFromResponse(
  res: Response,
): Promise<string> {
  ...
  try {
    ...
    if (json.error) message = translateAuthError(json.error);
      ...
      message = translateAuthError(json.message);
    else if (json.errors) {
      const parts = Array.isArray(json.errors)
        ? json.errors.map((e: unknown) => translateAuthError(String(e)))
        : Object.values(json.errors).map((e: unknown) =>
            translateAuthError(String(e)),
          );
      message = parts.join(", ");
    } else {
