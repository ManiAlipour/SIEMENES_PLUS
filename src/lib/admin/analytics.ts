...
type MonthAgg = { _id: { month: number }; total: number };

function buildMonthlySeries(items: MonthAgg[]): { month: string; views: number }[] {
...
async function aggregateDailyCounts(
  model: { aggregate: (pipeline: object[]) => Promise<{ _id: string; total: number }[]> },
  dateField: string,
  since: Date,
) {
...
function buildDailySeries(
  ...
  return Array.from({ length: days }, (_, i) => {
    ...
    const row: Record<string, string | number> = { day: label, date: key };
...
export async function getAdminAnalytics() {
  ...
  const [
    ...
    PageView.aggregate([
      { $group: { _id: { month: { $month: "$timestamp" } }, total: { $sum: 1 } } },
      { $sort: { "_id.month": 1 } },
    ]),
    ...
    UserSession.countDocuments({ loginAt: { $gte: oneDayAgo } }),
    UserSession.countDocuments({
      loginAt: { $gte: daysAgo(30) },
    }),
    Promise.all([
      User.countDocuments(),
    ...
    Promise.all([
      ...
      PageView.countDocuments({
        timestamp: { $gte: prevMonthStart, $lte: prevMonthEnd },
      }),
      ProductView.countDocuments({ timestamp: { $gte: monthStart } }),
  ...
  const dailyEngagement = buildDailySeries(7, {
    ...
    productViews: new Map(
      dailyProductViewsRaw.map((d) => [d._id, d.total]),
    ),
  ...
  const contentMix = [
    { label: "محصولات", value: contentCounts.totalProducts },
    { label: "مطالب وبلاگ", value: contentCounts.totalBlogPosts },
    { label: "ویدیو / پست", value: contentCounts.totalPosts },
    { label: "دسته‌بندی", value: contentCounts.totalCategories },
    { label: "نظرات", value: contentCounts.totalComments },
  ].filter((i) => i.value > 0);

  const engagementMix = [
    { label: "بازدید صفحات", value: totalViews },
    { label: "بازدید محصول", value: totalProductViews },
    { label: "جستجو", value: totalSearches },
    { label: "تعامل", value: totalInteractions },
    { label: "پیام تماس", value: contentCounts.totalContacts },
  ].filter((i) => i.value > 0);
  ...
  const viewsGrowth =
    periodCounts.pageViewsPrevMonth > 0
      ? (
          ((periodCounts.pageViewsThisMonth -
